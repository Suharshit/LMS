...
export const AttachmentForm = ({
  ...
  const onDelete = async(id: string) => {
    try {
      ...
      toast.error("Error, Attachment not deleted.")
    } finally {
      setDeletingId(null);
    }
  }
                ...
                {initialData.attachments.length > 0 && (
                  <div className="space-y-2">
                    {initialData.attachments.map((attachment) => (
                      <div key={attachment.id} className="flex items-center p-3 w-full bg-sky-100 border-sky-200 text-sky-700 border rounded-md">
